export const FETCH_CORONA_STATISTICS = 'FETCH_CORONA_STATISTICS';
export const SHOW_COUNTRY_STATISTICS = 'SHOW_COUNTRY_STATISTICS';
export const FETCH_MARKERS = 'FETCH_MARKERS';
export const MAP_STYLE= 'MAP_STYLE';
export const SET_ACTION = 'SET_ACTION';
export const MAP_STYLE_ACTION = 'MAP_STYLE_ACTION';
export const MAP_FLY_ACTION = 'MAP_FLY_ACTION';

export const BASE_URL = `https://intense-castle-52721.herokuapp.com/`;
// export const BASE_URL = `http://localhost:9000`;

// export const MAPBOX_ACCESS_TOKEN = `YOUR_MAPBOX_API_TOKEN`;
export const MAPBOX_ACCESS_TOKEN = `pk.eyJ1IjoicmF2aW5kcmFqaXRwdXJlIiwiYSI6ImNrYW1qMnQweTE0a3QzMHBpN3V0aG93Z2gifQ._e8p1pniJAvzXLPMuazBaw`;
// export const BASE_URL = `http://YOUR_LOCAL_IP_ADDRESS:9000`;
// export const MAPBOX_ACCESS_TOKEN = `YOUR_MAPBOX_API_TOKEN`;